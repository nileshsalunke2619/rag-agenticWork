...
4. Do NOT assume the current question relates to the previous topic just because it appeared right after it. A new question naming a different
person, system, term, product, or concept than the previous question is independent by default. Only merge context when the current question's own wording explicitly signals dependency on the previous one (a pronoun, "that", "this", "here", "what about X", or similar). When in doubt between "these are the same topic" and "these are differenttopics", prefer returning the question unchanged - an unnecessary reframe that narrows or distorts an independent question is worse than occasionally missing a genuine follow-up.
