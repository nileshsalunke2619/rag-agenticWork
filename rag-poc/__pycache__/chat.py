...
# This defines the actual POST /agent/v1/orchestrate/chat endpoint. It's
...
def ask(request: AskRequest) -> AskResponse:
    ...
    # NORMALIZE None -> "" right here, once - AskRequest now allows these
    # four fields to be explicit JSON null (see schemas.py's docstring
    # for why: a 422 was traced to the caller sending null instead of
    # omitting a field, which Pydantic rejects against a plain `str`
    # field). Doing this once, immediately, means every .strip()/logic
    # check below can keep assuming a plain string, same as before.
    request.userid = request.userid or ""
    request.questionid = request.questionid or ""
    request.isfollowup = request.isfollowup or ""
    request.lastquestion = request.lastquestion or ""
    ...
    if request.lastquestion.strip() and request.lastquestion.strip() != request.question.strip():
        ...
        logger.info(f'[{request_id}] [reframe] original questions is --- "{request.question}"')
        logger.info(f'[{request_id}] [reframe] llm reframe question is-- "{effective_question}"')
    else:
    ...
    if feedback_match is not None:
        ...
        response = AskResponse(answer=feedback_match["answer"], answeroption={})
        logger.info(
            f"[{request_id}] [response] Sending JSON response (feedback cache): "
    ...
    logger.info(
        f"[{request_id}] [response] Sending JSON response: "
        f"{json.dumps(response.model_dump(), ensure_ascii=False)}"
    )
    return response
