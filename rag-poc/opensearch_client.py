...
def search_top_chunks(
    ...
    there's no advanced score normalization pipeline here on purpose,
    to keep this simple to reason about and easy to tune later via
    ...
    (e.g. a chunk relevant to a DIFFERENT question asked earlier in the
    session). That leftover then gets handed to Claude as if it were
    real context, which is what caused one question's answer to bleed
    content from an entirely different previous question.
    ...
    chunks = [hit["_source"].get(config.TEXT_FIELD_NAME, "") for hit in kept_hits]
