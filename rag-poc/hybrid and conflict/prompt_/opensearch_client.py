...
def search_nearest_feedback(query_embedding: list, request_id: str = "") -> Optional[dict]:
    """
    WHAT THIS FUNCTION DOES:
    Looks up the SINGLE closest past thumbs-upped question in
    config.OPENSEARCH_FEEDBACK_INDEX, using a PURE kNN search (k=1) -
    deliberately NOT the hybrid kNN+BM25 query search_top_chunks() above
    uses. This index's "embedding" field is mapped with
    "space_type": "cosinesimil" (see app/scripts/create_feedback_index.py),
    ...
    RETURNS:
    None if the feedback index doesn't exist yet, is empty, or has no
    hits at all (all treated as "no feedback match" - the caller should
    fall through to the normal retrieve/generate flow, not error the
    request). Otherwise a dict: {"score": float, "question": str,
    "answer": dict} - "answer" is the exact answer JSON that was
    thumbs-upped, stored verbatim, ready to return as-is on a cache hit.
    """
    client = get_opensearch_client()
    ...
    try:
        ...
        # NOT_FOUND (index doesn't exist yet - e.g. before
        # create_feedback_index.py has been run, or before the first
    ...
    is_hit = score >= config.FEEDBACK_SIMILARITY_THRESHOLD
    logger.info(
        f"[{request_id}] [opensearch_client] Feedback check: nearest score={score:.4f} "
        f"(threshold={config.FEEDBACK_SIMILARITY_THRESHOLD}) -> "
        f"{'CACHE HIT' if is_hit else 'MISS, continuing to normal retrieval'}"
    )
    ...
    return {
        ...
        "answer": source.get("responseJSON", {}),
    }
...
def store_feedback(
    ...
    acknowledged but never reach this function). Stores the question's
    embedding (for future kNN matching), plus enough metadata to trace
    which user/question this came from, plus the exact answer JSON that
    was thumbs-upped (returned verbatim on a future cache hit).
    """
    client = get_opensearch_client()
    ...
    document = {
        ...
        "responseJSON": payload.get("responseJSON", {}),
    }
    ...
    logger.info(
        ...
        f"{config.OPENSEARCH_FEEDBACK_INDEX!r} for questionid="
