...
"documentids" (the document_id counterpart to "references"): for every entry in "references", populate the SAME-POSITION entry in "documentids" with that same document's document_id from its label - "documentids"[0] must be the document_id for "references"[0], "documentids"[1] for "references"[1], and so on. Both arrays MUST always be the exact same length. If a chunk's label has no document_id, use "" for that position in "documentids" - do not skip the position, do not fabricate an id.
...
4. Every key listed in JSON STRUCTURE below MUST always be present in "answer", even when not applicable. Use these exact "empty" values when a field doesn't apply: "header": "", "subheader": "", "references": [], "documentids": [], "followupquestions": [], "steps": []. "documentids" must always be the exact same length as "references".
...
EXAMPLE 1 - greeting:
{
  "answer": {
    ...
    "header": "", "subheader": "", "references": [], "documentids": [], "followupquestions": [], "steps": []
  }
}
...
EXAMPLE 3 - insufficient context:
{
  "answer": {
    ...
    "references": ["Referenced Document"], "documentids": ["DOC-REF-009"], "followupquestions": [], "steps": []
  }
}
...
EXAMPLE 5 - relevant answer, but the chunk had NO usable source label:
{
  "answer": {
    ...
    "references": [], "documentids": [], "followupquestions": [], "steps": []
  }
}
...
EXAMPLE 6 - Case 1, different contexts, not contradictory (based on: "what documents are required for a shipment?"):
{
  "answer": {
    ...
    "documentids": ["DOCID-104302", "DOCID-130689"],
...
EXAMPLE 8 - vague input, using followupquestions to offer concrete options instead of a generic "please clarify":
{
  "answer": {
    ...
    "header": "", "subheader": "", "references": [], "documentids": [],
...
Before responding, check: every key present, no placeholder text, every reference traceable to a real label of a chunk actually used, "documentids" is the EXACT same length as "references" with matching positions.
"""
)
...
"documentids" (the document_id counterpart to "references"): for every entry in "references", populate the SAME-POSITION entry in "documentids" with that same document's document_id from its label. Both arrays MUST always be the exact same length. If a chunk's label has no document_id, use "" for that position - do not skip the position, do not fabricate an id.
...
2. Return your output as a single valid JSON object - never as a JSON string, never double-escaped, never wrapped inside another string value.
3. The JSON must be syntactically valid - no trailing commas, all strings properly escaped and quoted.
4. Every key listed in JSON STRUCTURE below MUST always be present in BOTH "answer" and "answeroption". Use these exact "empty" values when a field doesn't apply: "header": "", "subheader": "", "references": [], "documentids": [], "followupquestions": [], "steps": []. "documentids" must always be the exact same length as "references" within each object.
...
EXAMPLE - based on "does a serialized product need to be decommissioned before shipment?":
{
  "answer": {
    ...
    "references": ["SOP-104302"], "documentids": ["DOCID-104302"], "followupquestions": [], "steps": []
  },
  "answeroption": {
    ...
    "references": ["SOP-129268"], "documentids": ["DOCID-129268"], "followupquestions": [], "steps": []
  }
}
NOTE: each answer object contains ONLY its own source's position - content is never blended between them.

Before responding, check: both "answer" and "answeroption" are populated with genuinely independent, non-blended content, every reference traceable to a real label, "documentids" matches "references" length in each object, and "answer.description" states plainly that the sources disagree.
"""
)
