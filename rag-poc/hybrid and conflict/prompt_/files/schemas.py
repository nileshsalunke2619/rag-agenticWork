...
class AskRequest(BaseModel):
    ...
    NEW FIELDS (query reframing): userid/questionid are accepted for
    traceability but not used functionally today. isfollowup/lastquestion
    drive query reframing (see app/api/routes/chat.py) - reframing only
    runs when isfollowup == "y" AND lastquestion is non-empty. All four
    default to "" so older callers sending just {"question": "..."}
    still validate fine and simply never trigger reframing.

    Example valid request body WITH reframing fields:
        {
          "userid": "akhilesh.kumar2@pfizer.com",
          "question": "What will be more steps here",
          "questionid": "1788159501808_akhilesh.kumar2@pfizer.com",
          "isfollowup": "y",
          "lastquestion": "How to create sales order"
        }
    """
