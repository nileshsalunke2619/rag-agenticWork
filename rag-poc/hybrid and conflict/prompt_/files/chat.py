...
def ask(request: AskRequest) -> AskResponse:
    ...
    3. QUERY REFRAMING (new): if request.isfollowup == "y" AND
       request.lastquestion is non-empty and different from the current
       question, calls llm.reframe_query() to rewrite the follow-up
       question into a standalone one BEFORE retrieval - e.g. "what will
       be more steps here" (with lastquestion "How to create sales
       order") becomes "What are the remaining steps to create a sales
       order?". Every step after this uses the REFRAMED question, not
       the raw one. This is fully fail-safe: if reframing fails or
       returns something unusable, reframe_query() itself falls back to
       the original question - this endpoint never has to handle that
       case specially. See app/llm/bedrock_client.py's reframe_query()
       and app/prompts/system_prompt.py's QUERY_REFRAMING_PROMPT.
    ...
    # QUERY REFRAMING - only when the caller explicitly says this is a
    # follow-up AND actually sent a previous question to reframe against.
    # Guards against pointless/no-op reframe calls: a fresh question
    # (isfollowup != "y"), no lastquestion sent, or lastquestion being
    # identical to the current question (nothing to resolve) all skip
    # straight past this and use the question as-is.
    effective_question = request.question
    if (
        request.isfollowup == "y"
        and request.lastquestion.strip()
        and request.lastquestion.strip() != request.question.strip()
    ):
