...
def _format_chunk_with_source(chunk: dict) -> str:
    ...
    cite a REAL source in "references" instead of guessing from the
    chunk's raw body text - see the "SOURCE LABELS AND CITATIONS"
    section of app/prompts/system_prompt.py, which expects EXACTLY this
    "[Source - document: ..., document_id: ..., section: ..., page: ...]"
    format. The document_id in this label is also what
    _attach_document_ids() uses to append the real ID onto each
    reference after the fact - Claude never has to type the ID itself.
...
def _filter_hallucinated_references(answer: dict, chunks: list, request_id: str = "") -> None:
    ...
    despite the system prompt's rules - modifies answer["references"] IN
    PLACE, keeping only entries whose reference actually starts with a
    real document name from the chunks retrieved for THIS request.
    Prompt instructions alone are necessary but not sufficient - a model
    can still slip - so this makes it structurally impossible for a
    made-up reference to reach the caller, regardless of whether Claude
    followed the prompt correctly. Never makes the list LONGER - only
    ever removes entries that don't check out.

    Runs BEFORE _attach_document_ids() - references are still clean,
    Claude-authored document names here, not yet suffixed with an ID.
    """
    ...
    #
    # .strip() on both sides: some ingested document_name values carry
    # stray leading/trailing whitespace (e.g. " SOP-IT-SEC-001_..." with
    # a leading space - a real ingestion data-quality issue, not
    # something this file controls). Without stripping, a completely
    # legitimate reference gets rejected as "hallucinated" the moment
    # Claude's reference text doesn't happen to reproduce that stray
    # whitespace verbatim, which isn't something we can rely on Claude
    # doing consistently. Stripping makes the match tolerant of that
    # inconsistency without weakening the check - a fabricated
    # reference still won't match any real document_name regardless.
    known_document_names = {
        chunk["document_name"].strip()
        for chunk in chunks
        if chunk.get("document_name") and chunk["document_name"].strip()
    }
    ...
    kept_references = [
        ...
        if any(reference.strip().startswith(name) for name in known_document_names)
    ]
...
def _deduplicate_references(answer: dict, request_id: str = "") -> None:
    ...
    Runs BEFORE _attach_document_ids() - deduplicating on the clean,
    Claude-authored document name is what makes this reliable. If an ID
    suffix were already attached, any formatting drift Claude introduced
    while retyping the ID would make two mentions of the SAME document
    look like different strings and silently break this dedup - exactly
    the bug we fixed once already by keeping references name-only until
    the very last step.
    """
...
def _attach_document_ids(answer: dict, chunks: list, request_id: str = "") -> None:
    """
    WHAT THIS FUNCTION DOES:
    Appends " <document_id>" to the end of each entry in
    answer["references"] IN PLACE, e.g. "SOP - EU consignment Transfers"
    becomes "SOP - EU consignment Transfers SOP-123693" - per the
    required response format.

    WHY THIS RUNS LAST, AND WHY THE ID IS NEVER CLAUDE-GENERATED:
    By this point references has already been hallucination-filtered and
    deduplicated while still clean/name-only (see the two functions
    above) - the most reliable state that data is ever in. The document_id
    appended here comes ONLY from the actual retrieved chunk's own
    metadata (ground truth from OpenSearch/ingestion), NEVER from
    anything Claude typed - Claude is never asked to produce an ID at
    all. This makes the ID portion of every reference structurally
    impossible to hallucinate, since it is not LLM-generated in the
    first place.
    """
    name_to_id = {}
    for chunk in chunks:
        ...
        if name and name not in name_to_id:
            name_to_id[name] = chunk.get("document_id") or ""

    combined_references = []
    for reference in answer.get("references", []):
        document_id = next(
            (doc_id for name, doc_id in name_to_id.items() if reference.strip().startswith(name)),
            "",
        )
        combined_references.append(f"{reference} {document_id}".strip() if document_id else reference)

    answer["references"] = combined_references


def reframe_query(question: str, lastquestion: str, request_id: str = "") -> str:
    """
    WHAT THIS FUNCTION DOES:
    Called from app/api/routes/chat.py, ONLY when the request's
    isfollowup == "y" and lastquestion is non-empty. Makes ONE small
    Claude call using QUERY_REFRAMING_PROMPT to rewrite a follow-up
    question (e.g. "what will be more steps here") into a standalone
    question (e.g. "what are the remaining steps to create a sales
    order?") that can be searched against the document index on its own.

    NOT BUILT ON get_raw_claude_text(): that function's user-prompt shape
    is "Context: <chunks> / Question: <question>" - built for the
    retrieval-grounded prompts (detection/answer generation). Reframing
    has no chunks and needs BOTH the previous and current question, so
    this makes its own minimal invoke_model() call instead of forcing
    that shape to fit.

    FAIL-SAFE, BY DESIGN - reframing must NEVER be able to block or break
    a request:
    - Any exception during the Bedrock call (network, throttling, auth)
      is caught, logged, and falls back to returning the ORIGINAL
      question unchanged.
    - The response is sanity-checked: empty (after stripping) or
      unreasonably long (over 500 characters - a real rewritten question
      is a short sentence, not a paragraph) also falls back to the
      original question, on the assumption something went wrong rather
      than trusting a malformed rewrite.
    - There is deliberately NO JSON parsing here (unlike detect_conflict/
      ask_claude_single/ask_claude_conflict) - the prompt asks for plain
      text specifically so a malformed-JSON failure mode isn't even
      possible for this call.
    """
    user_prompt = f"Previous question: {lastquestion}\nCurrent question: {question}"

    request_body = json.dumps(
        {
            "anthropic_version": "bedrock-2023-05-31",
            "max_tokens": 1000,
            "system": QUERY_REFRAMING_PROMPT,
            "messages": [
                {"role": "user", "content": user_prompt},
            ],
        }
    ...
    try:
        ...
        stop_reason = response_body.get("stop_reason")
        logger.info(
            f"[{request_id}] [bedrock_client] Reframe call took {duration:.2f}s, "
            f"stop_reason={stop_reason!r}"
        )
        ...
        for block in response_body.get("content", []):
            if block.get("type") == "text":
                raw_text = block["text"]
                break
    except Exception:  # noqa: BLE001 - reframing must never break the request
        logger.exception(
            f"[{request_id}] [bedrock_client] Reframe call FAILED - falling back to "
            f"original question {question!r}"
        )
        return question

    reframed = raw_text.strip()

    if not reframed or len(reframed) > 500:
        logger.warning(
            f"[{request_id}] [bedrock_client] Reframed question failed a sanity "
            f"check (empty or unreasonably long) - falling back to original "
            f"question. Raw text was: {raw_text!r}"
        )
        return question

    logger.info(
        f"[{request_id}] [bedrock_client] Reframed {question!r} (using last "
        f"question {lastquestion!r}) -> {reframed!r}"
    )
    return reframed
...
def detect_conflict(question: str, chunks: list, request_id: str = "") -> bool:
    ...
    raw_text = get_raw_claude_text(
        question, chunks, CONFLICT_DETECTION_PROMPT, max_tokens=1024, request_id=request_id
    )
    try:
...
def ask_claude_single(question: str, chunks: list, request_id: str = "") -> dict:
    ...
    _attach_document_ids(answer, chunks, request_id)
    return answer
...
def ask_claude_conflict(question: str, chunks: list, request_id: str = "") -> tuple:
    ...
    if answeroption:
        ...
        _attach_document_ids(answeroption, chunks, request_id)
    else:
        logger.warning(
