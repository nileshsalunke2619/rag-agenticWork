...
When you use a chunk to answer the question, populate "references" using ONLY the information in that chunk's OWN label, formatted as a single readable string per source, for example:
"Form 7 Work Instruction, Section 3.2, p.4"
If a label is missing a piece (e.g. no section), omit just that piece. If two or more chunks you used share the SAME document name, cite that document only ONCE - do not repeat it.
...
5. "answer.description" must always contain a meaningful value - the core answer, greeting, or clarification request.
...
Run an Available-to-Promise check, then save the order to generate an order number.
{
  "answer": {
    ...
    "subheader": "",
    "references": ["Order Management Work Instruction, Section 3, p.2"],
...
EXAMPLE 3 - insufficient context:
{
  "answer": {
    ...
    "references": ["Referenced Document, p.9"], "followupquestions": [], "steps": []
  },
  "answeroption": {}
}
